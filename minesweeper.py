...
class minesweeper(commands.Cog):
    ...
    @commands.slash_command(name="campominado", description="Jogue campominado!")
    async def campominado(self, ctx, columns=None, rows=None, bombs=None):
        ...
        if columns > 13 or rows > 13:
            await ctx.send('O limite para colunas e linhas é 13.')
            return
        if columns < 1 or rows < 1 or bombs < 1:
            await ctx.send('O número informado não pode ser menor que 1.')
            return
        if bombs + 1 > columns * rows:
            await ctx.send(':boom:**BOOM**, você colocou mais bombas do que espaços disponíveis.')
            return
        ...
        embed = disnake.Embed(title='\U0001F642 Campo Minado \U0001F635', color=0xC0C0C0)
        embed.add_field(name='Colunas:', value=columns, inline=True)
        embed.add_field(name='Linhas:', value=rows, inline=True)
        embed.add_field(name='Total de espaços:', value=columns * rows, inline=True)
        embed.add_field(name=':boom: Bombas:', value=bombs, inline=True)
        embed.add_field(name=':boom: Porcentagem:', value=f'{percentage}%', inline=True)
